...
export function NotificationBell() {
    ...
    </Link>{toast === organizationId && count > 0 && <div role="status" className="fixed end-4 top-20 z-50 flex items-center gap-4 rounded-2xl border border-emerald-200 bg-white p-4 shadow-xl"><Link href="/app/notifications" onClick={() => setToast(null)} className="text-sm text-emerald-800">{t('notifications.count', { count })}</Link><button type="button" onClick={() => setToast(null)} aria-label="Close / إغلاق" className="rounded-full px-2 py-1">×</button></div>}</>;
}
