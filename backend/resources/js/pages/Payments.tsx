...
function PaymentsWorkspace() {
    ...
    const canManage = permissions ? permissions.includes('payments.manage') : allowed;
    ...
    return <AppShell><Head title={t('payments.title')} /><main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-8">
        ...
        {!allowed ? <p>{t('payments.noAccess')}</p> : <>
            {canManage && <details className="group rounded-2xl border border-[var(--ac-line)] bg-white p-5 shadow-sm open:border-[var(--ac-accent)] sm:p-6">
                ...
                <form onSubmit={create} className="mt-5"><fieldset disabled={busy} className="grid gap-4 sm:grid-cols-2">
                    ...
                    <ChoiceField name="frequency" label={t('payments.frequency')} defaultValue="once" onChange={setFrequency} options={(['once', 'weekly', 'monthly', 'yearly'] as const).map((value) => ({ value, label: t(`payments.${value}`) }))} />
