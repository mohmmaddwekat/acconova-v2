...
final class BillingAddonService
{
    ...
    public function purchase(
        ...
        $this->persistResult((int) $organization->id, $addonKey, $result);

        return $result;
    }

    /**
     * Set an add-on to the exact requested quantity. This supports upgrade,
     * downgrade and removal (quantity zero) on the same Stripe subscription.
     *
     * @return array<string, mixed>
     */
    public function setQuantity(
        ...
        $account = $this->account($organization);
        $result = $this->subscriptions->setAddonQuantity(
            $account,
            $addonKey,
    ...
    public function activeForOrganization(int $organizationId): array
    {
        ...
        foreach ($rows as $row) {
            ...
            $providerQuantity = max(0, (int) $row->quantity);
            $perUnit = max(0, (int) ($addon['quantity'] ?? 0));
            $active[(string) $row->addon_key] = [
                'quantity' => $providerQuantity,
                'entitlement' => $providerQuantity * $perUnit,
    ...
    public function publicCatalog(?string $interval = null, int $organizationId = 0): array
    {
        ...
        foreach ((array) config('billing_growth.addons', []) as $key => $addon) {
            ...
            $unitAmount = max(0, (int) ($price['amount_minor'] ?? 0));
            $unitEntitlement = max(0, (int) ($addon['quantity'] ?? 0));
            $displayBundleQuantity = max(1, (int) ($addon['display_bundle_quantity'] ?? 1));
            $displayAmount = $unitAmount * $displayBundleQuantity;
            $displayEntitlement = $unitEntitlement * $displayBundleQuantity;
            $activeQuantity = (int) ($active[$key]['quantity'] ?? 0);
            ...
            $catalog[] = [
                ...
                'quantity_per_pack' => $displayEntitlement,
                'unit_quantity' => $unitEntitlement,
                'display_bundle_quantity' => $displayBundleQuantity,
                'amount_minor' => $displayAmount,
                'unit_amount_minor' => $unitAmount,
                'currency' => strtoupper((string) ($addon['currency'] ?? 'USD')),
                'interval' => $interval,
                'active_quantity' => $activeQuantity,
                'active_entitlement' => (int) ($active[$key]['entitlement'] ?? 0),
                'max_quantity' => max(1, (int) ($addon['max_quantity'] ?? 100)),
                'available' => $unitAmount > 0,
            ];
        }
